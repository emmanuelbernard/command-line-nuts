...
#
# History:
# 2013-05-13, Emmanuel Bernard <code@mel.emmanuelbernard.com>
#     version 1.2: Added expansion of command string with message, nick and
#                  weechat home.
#                  $weechat_message, $weechat_nick and $weechat_home are the expanded templates
        ...
        if ($options{beep_highlight} eq "on")
        {
            if ($options{whitelist_channels} ne "")
            {
                ...
                if ($options{beep_trigger_highlight} eq "" or $message =~ m/\b$options{beep_trigger_highlight}\b/)
                {
                    ...
                    {
                        beep_exec_command($options{beep_command_highlight}, $message, $nick);
                    }
...
sub beep_trigger_whitelist_blacklist
{
    ...
    if ($trigger eq "" or $message =~ m/\b$trigger\b/)
    {
        ...
        if ($whitelist eq "on" and $options{whitelist_nicks} ne "")
        {
            ...
            {
                beep_exec_command($command, $message, $nick);
            }
...
{
    my ($command, $message, $nick) = @_;
    if ($command =~ /^\$bell/)
    {
        print STDERR "\a";
        ($command) = $command =~ /^\$bell;(.+)$/;
    }
    # expand $weechat_message, $weechat_nick and $weechat_home
    $command =~ s/\$weechat_message/$message/gi if ($message ne "");
    $command =~ s/\$weechat_nick/$nick/gi if ($nick ne "");
    my $weechat_home = weechat::info_get("weechat_dir", "");
    $command =~ s/\$weechat_home/$weechat_home/gi;
